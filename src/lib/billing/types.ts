...
export type BillingDocument = { id: string; service_job_id?:string|null; cancelled_at?:string|null; kind: 'quotation'|'invoice'; number: string; customer_id: string; customer_snapshot: {name:string;phone:string;village:string;address?:string;gstin?:string}; seller_snapshot: BusinessProfile; document_date: string; valid_until: string|null; items: BillingLine[]; subtotal: number; tax_amount: number; total: number; amount_paid: number; tax_mode: 'cgst_sgst'|'igst'; notes: string; source_quotation_id: string|null; created_at: string; updated_at:string };
