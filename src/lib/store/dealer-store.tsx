...
import type { Product, BillingDocument, BusinessProfile, DocumentInput } from '@/lib/billing/types';
type Row = Record<string, any>;
type State = { products: Product[]; documents: BillingDocument[]; profile: BusinessProfile|null; customers: Customer[]; inventory: InventoryItem[]; physicalVehicles: PhysicalVehicle[]; vehicleModels: VehicleModel[]; implementsList: Implement[]; parts: SparePart[]; batteries: Battery[]; quotations: Quotation[]; invoices: Invoice[]; payments: Payment[]; serviceJobs: ServiceJob[]; pdiChecks: PDICheck[]; warranties: Warranty[]; timeline: CustomerTimelineEvent[]; movements: Row[] };
const empty: State = { products:[],documents:[],profile:null,customers:[],inventory:[],physicalVehicles:[],vehicleModels:[],implementsList:[],parts:[],batteries:[],quotations:[],invoices:[],payments:[],serviceJobs:[],pdiChecks:[],warranties:[],timeline:[],movements:[] };
...
export function DealerStoreProvider({children}:{children:React.ReactNode}){
 ...
 const refresh=useCallback(async()=>{
  ...
  try{
   const [products,customers,documents,profile,inventory,payments,jobs,pdi,warranties,timeline,movements,vehicles]=await Promise.all([
   ...
   next.invoices=next.documents.filter(d=>d.kind==='invoice').map(d=>({id:d.id,saleId:'',invoiceNo:d.number,customerId:d.customer_id,customerName:d.customer_snapshot.name,customerPhone:d.customer_snapshot.phone,customerVillage:d.customer_snapshot.village,items:items(d),subtotal:d.subtotal,discount:0,cgstAmount:d.tax_mode==='cgst_sgst'?d.tax_amount/2:0,sgstAmount:d.tax_mode==='cgst_sgst'?d.tax_amount/2:0,igstAmount:d.tax_mode==='igst'?d.tax_amount:0,totalAmount:d.total,amountPaid:d.amount_paid,balanceDue:d.total-d.amount_paid,createdAt:d.created_at}));
